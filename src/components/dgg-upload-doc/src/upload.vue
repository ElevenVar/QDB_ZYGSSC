...
export default {
  ...
  methods: {
    ...
    handleChange(ev) {
      ...
      if (this.selectName.includes(_files[0].name)) {
        this.$message({
          message: `请不要重复选取同名文件`,
          type: "warning",
      ...
      this.selectName.push(_files[0].name);
    ...
    setSelectName() {
      this.selectName = this.fileList.map(item => item.name);
    },
    uploadFiles(files) {
    ...
    manualUpload(callBack) {
      ...
      let postFiles = Array.prototype.slice.call(this.fileList);
      if (!this.multiple) {
        postFiles = postFiles.slice(0, 1);
      }
      if (postFiles.length === 0) {
      ...
      postFiles.forEach(async rawFile => {
        let _promise = new Promise(async resolve => {
          // await this.upload(rawFile.raw);
          resolve(await this.upload(rawFile.raw));
        });
        _arr.push(_promise);
      });
      Promise.all(_arr).then(result => {
        if (result.every(item => !item)) return;
