 ...
 * @param headers 头信息
 */
function sendGetRequest(url, sysCode, secret, callBack, headers) {
    ...
    var sign = createSign(sortData, sysCode, secrect);
    $.ajax({
        type: "GET",
        url: url,
        beforeSend: function(request) {
            ...
            request.setRequestHeader("sysCode", sysCode);
            request.setRequestHeader("sign", sign);
            request.setRequestHeader("replaceEmoji", "true");
            if (headers) {
                for (var headerKey in headers) {
                    //空值不传递，不参与签名组串
                    var v = headers[headerKey];
                    request.setRequestHeader(headerKey, v);
                }
            }
 ...
 * @param headers 头信息
 */
function sendFromRequest(formId, url, sysCode, secret, callBack, headers) {
    var reqObj = $("#" + formId).serializeObject(); //将表单序列化为JSON对象
    //将json排序
    reqObj = sort_ASCII(reqObj);
    //将排序后的json转为a=b&c=d形式字符串
    var formStr = getFormStr(reqObj);
...
/**
 *
 * @param reqObj 请求json对象
 * @param url  请求url
 * @param sysCode 系统sysCode
 * @param secrect 系统秘钥
 * @param callBack 回调放方法
 * @param headers 头信息
 */
function sendJsonRequest(reqObj, url, sysCode, secrect, callBack, headers) {
    var time = new Date().getTime();
    var nonce = createRandomId();
    var sortData = { "nonce": nonce, "time": time, "data": JSON.stringify(reqObj) };
    var sign = createSign(sortData, sysCode, secrect);
    $.ajax({
        type: "POST",
        url: url,
