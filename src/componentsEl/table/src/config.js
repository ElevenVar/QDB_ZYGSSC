import { getPropByPath } from '@/componentsEl/src/utils/util';
...
export const cellForced = {
  ...
  expand: {
    ...
    renderCell: function(h, { row,column, store }) {
      ...
      if (store.states.expandRows.indexOf(row) > -1 && !column.expandRender) {
      ...
      return (<div class={column.expandRender ? '' : classes }
        on-click={callback}>
        { column.expandRender ?  column.expandRender(h, row) :<i class='el-icon el-icon-arrow-right'></i>} 
      </div>);
    },
    sortable: false,
    resizable: false,
